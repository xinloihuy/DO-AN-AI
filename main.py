...
game_over = False
def next_map():
    global level, player,all_sprite,map, enemy,x_enemy,y_enemy
    all_sprite.empty() 
    enemies_group.empty()
    gold_group.empty()
    map = Map()
...
def reset_game():
    global score, level, player, enemy, rocket, map, all_sprite, enemies_group, gold_group
...
def draw():
    ...
    if player.game_over == False:
        ...
        #rocket.check_rocket(screen,player,150,camera)
